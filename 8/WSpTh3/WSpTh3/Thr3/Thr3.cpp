...
unsigned int __stdcall  PaintTriangle(void *hWnd);

void SuspendEllipse(HMENU hMenu,bool *bSuspend);
void SuspendRectangle(HMENU hMenu,bool *bSuspend);
void SuspendTriangle(HMENU hMenu,bool *bSuspend);
...
void TerminateTriangle(HWND hWnd,HMENU hMenu,BOOL *fTerminateT);
...
LONG WndProc_OnCommand (HWND hWnd,int id,HWND hwndCtl,UINT codeNotify)
{
		// Parse the menu selections:
			switch (id)
			{
				...
				case IDM_SUST:
				SuspendTriangle(GetMenu(hWnd),&bSuspendT);
				return 0L;
				...
				case IDM_TERMT:
				TerminateTriangle(hWnd,GetMenu(hWnd), &fTerminateT);
				return 0L;
...
LONG WndProc_OnDestroy(HWND hWnd)
{
	...
	{MessageBox(NULL,"CloseHandle  failed",
					       "PaintTriangle Thread", MB_OK|MB_ICONEXCLAMATION);
	};
...
unsigned int __stdcall  PaintTriangle(void *hWnd){
	...
	// To be continued!
		EnterCriticalSection(&cs);
				...
				GetWindowRect((HWND)hWnd,&rect);
				x1.x=rand()%(rect.right +1);
				x1.y=rand()%(rect.bottom +1);
				x2.x=rand()%(rect.right +1);
				x2.y=rand()%(rect.bottom +1);
				x3.x=rand()%(rect.right +1);
				x3.y=rand()%(rect.bottom +1);
				
				hBrush=CreateSolidBrush(RGB(nRed,nGreen,nBlue));
				hOldBrush=(HBRUSH)SelectObject(hDC,hBrush);
				const POINT arr[] = {x1, x2, x3};
				Polygon(hDC, arr, 3);
				SelectObject(hDC,hOldBrush);
				DeleteObject(hBrush);
				ReleaseDC((HWND)hWnd,hDC);
...
switch(dwRetCode=WaitForSingleObject(hMutex,INFINITE))
{
	...
	if(!*bSuspend){	 
		...
		{		
			wsprintf(message,TEXT("SuspendThread Error %ld"),GetLastError());
			MessageBox(NULL,message,"PaintTriangle Thread",MB_OK|MB_ICONEXCLAMATION);
			ReleaseMutex(hMutex);
			return;
		}
		...
		*bSuspend = true;
		//Check
		//Sets the check-mark attribute to the checked state.
		CheckMenuItem(hMenu,IDM_SUST,MF_CHECKED);
	}
	else
	{	
		...
		{			
			wsprintf(message,TEXT("ResumeThread Error %ld"),GetLastError());
			MessageBox(NULL,message,"PaintTrinagle Thread",MB_OK|MB_ICONEXCLAMATION);
			ReleaseMutex(hMutex);
			return;
		}
		...
		*bSuspend = false;
		//Uncheck 
		//Sets the check-mark attribute to the unchecked state.
		CheckMenuItem(hMenu,IDM_SUST,MF_UNCHECKED);			
	}
	ReleaseMutex(hMutex);
	return;
	break;
